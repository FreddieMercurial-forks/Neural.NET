...
public class NetworkTrainer
{
    ...
    /// </summary>
    private FullyConnectedNetwork Network { get; }
    ...
    /// </param>
    public IEnumerable<Tuple<int, double?>> StochasticGradientDescent(int trainingSize, Tuple<double[], double[]>[] trainingData,
        int epochs, int miniBatchSize, double learningRate, Tuple<double[], double[]>[] testData = null)
    {
        // Convert the double array into something a we like more (vectors)
        var _trainingData = new Tuple<Vector<double>, Vector<double>>[trainingData.Length];
        var _testData = testData == null ? null : new Tuple<Vector<double>, Vector<double>>[testData.Length];
        ...
        for (var i = 0; i < _trainingData.Length; i++)
        {
            _trainingData[i] = new Tuple<Vector<double>, Vector<double>>(
                item1: Vector<double>.Build.DenseOfArray(array: trainingData[i].Item1),
                item2: Vector<double>.Build.DenseOfArray(array: trainingData[i].Item2));
        }
        ...
        if (_testData != null)
        {
            ...
            for (var i = 0; i < _testData.Length; i++)
            {
                _testData[i] = new Tuple<Vector<double>, Vector<double>>(
                    item1: Vector<double>.Build.DenseOfArray(array: testData[i].Item1),
                    item2: Vector<double>.Build.DenseOfArray(array: testData[i].Item2));
            }
    ...
    {
        var _nablaB = new Vector<double>[this.Network.Biases.Count];
        var _nablaW = new Matrix<double>[this.Network.Weights.Count];

        // Feed forward
        var _activation = input;

        // List to store all activations, layer by layer
        var _activations = new List<Vector<double>>(capacity: this.Network.LayerCount);
        _activations.Add(item: _activation);

        // Store all the z vectors (outputs of each layer before running the activation function)
        var _zOutputs = new List<Vector<double>>(capacity: this.Network.LayerCount);
        ...
        {
            var _z = this.Network.Weights[index: i].Multiply(rightSide: _activation).Add(other: this.Network.Biases[index: i]);
            _zOutputs.Add(item: _z);
            _activation = this.Network.RunActivation(activation: _z,
        ...
        // middle layers
        for (var i = this.Network.LayerCount - 2; i > 0; i--)
        {
    ...
    {
        return actual.Subtract(other: expected);
    }
    ...
    private double Evaluate(Tuple<Vector<double>, Vector<double>>[] testData)
    {
        ...
        var _testResults = new List<Tuple<int, int>>(capacity: testData.Length);

        foreach (var _singleRun in testData)
        {
            // For each run we store off the maximum index of what the network thought was the
            // answer, as well as the actual answer
            _testResults.Add(item: new Tuple<int, int>(item1: this.FeedForward(activation: _singleRun.Item1).MaximumIndex(),
                item2: _singleRun.Item2.MaximumIndex()));
        }

        // We return back the count of how many were correct. This is a double so we can make it
        // a percentage easier.
        return _testResults.Where(predicate: a => a.Item1 == a.Item2).Count();
    }
    ...
    private Vector<double> FeedForward(Vector<double> activation)
    {
        ...
        for (var i = 0; i < this.Network.LayerCount; i++)
        {
            activation = this.Network.RunActivation(
                activation: this.Network.Weights[index: i].Multiply(rightSide: activation).Add(other: this.Network.Biases[index: i]),
                activationFunction: this.Network.LayerInformation[index: i].ActivationFunction);
        }
    ...
    private void UpdateMiniBatch(Tuple<Vector<double>, Vector<double>>[] batch, double learningRate)
    {
        ...
        foreach (var pair in batch)
        {
            ...
            lock (this._sync)
            {
                ...
                for (var i = 0; i < _nablaB.Length; i++)
                {
                    _nablaB[i] = _nablaB[i].Add(other: _deltas.Item1[i]);
                }

                //_nablaB = _nablaB.Zip(_deltas.Item1, (nb, dnb) => nb.Add(dnb)).ToArray();
                for (var i = 0; i < _nablaW.Length; i++)
                {
                    _nablaW[i] = _nablaW[i].Add(other: _deltas.Item2[i]);
                }
            }
        }
        ...
        for (var i = 0; i < this.Network.LayerCount; i++)
        {
            this.Network.Weights[index: i] =
                this.Network.Weights[index: i].Subtract(other: _nablaW[i].Multiply(scalar: learningRate / batch.Length));
        }

        for (var i = 0; i < this.Network.LayerCount; i++)
        {
            this.Network.Biases[index: i] =
                this.Network.Biases[index: i].Subtract(other: _nablaB[i].Multiply(scalar: learningRate / batch.Length));
        }
    }
}
